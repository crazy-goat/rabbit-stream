 ...
 *
 *   ### Some entry title
 *   <!-- id=FAQ-001 date=2025-01-01 tags=foo,bar trigger="when X" hits=0 status=active -->
 *
 * Grammar: <key>=<value>, all on one line. A value is either bare (no
 * whitespace) or double-quoted. Required keys: id, date, tags, trigger, hits,
 * status. Optional: gate (mandatory for `status=promoted`).
 *
 ...
 * Jaccard) is used on purpose — a short entry fully contained in a long one
 * is a duplicate even though Jaccard would score it low.
 */
...
const STOP_WORDS = [
    ...
    'or', 'that', 'the', 'this', 'to', 'was', 'were', 'when', 'which', 'with',
    'you', 'your',
];
...
function parseFrontMatter(string $line): array
{
    ...
    if (preg_match('/^<!--\s+(.*)\s+-->$/', $trimmed, $matches) !== 1) {
        return ['pairs' => [], 'error' => 'front matter must be a single-line "<!-- … -->" comment'];
    }
...
function parseFile(string $relative, string $absolute): array
{
    ...
    /** @var ?list<string> $currentBody reference to the body of the entry being collected */
    $currentBody = null;
 ...
 *
 * @param list<Entry>                                   $entries
 * @param ?array{start: int, end: int, body: list<string>, footprint: int} $index
 */
...
function main(array $options): int
{
    ...
    foreach ($stale as $entry) {
        fwrite(
            STDOUT,
            sprintf("kb-lint: stale (0 hits in %d cycles, remove at the next retro): %s\n", STALE_AFTER_CYCLES, $entry),
        );
    }
